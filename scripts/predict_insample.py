...
INSAMPLE_STEPS = [
    ("deposition", 1, "init", "1M"),
    ("etch", 1, "1M", "1E"),
    ("deposition", 2, "1E", "2M"),
    ("etch", 2, "2M", "2E"),
]
INSAMPLE_TARGET_STATES = ["1M", "1E", "2M", "2E"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Predict one-step in-sample transitions with GT inputs.")
    ...
    parser.add_argument("--no-plot", action="store_true", help="Only save prediction artifacts")
    parser.add_argument(
        "--contour-mode",
    ...
    prediction_dir = Path(args.output_dir) if args.output_dir else out_dir / "insample_predictions"
    ...
    predictions = {}
    manifest_rows = []
    for process_name, cycle, input_state, target_state in INSAMPLE_STEPS:
        if input_state not in states:
            raise KeyError(f"Missing input state {input_state!r} in configured state_sources")
        ...
        phi_pred = predict_next_levelset(phi_initial, duration, rate, process_sign, config, model, device=device, dtype=dtype)
        predictions[target_state] = phi_pred
        np.save(prediction_dir / f"{target_state}.npy", phi_pred)
        np.save(prediction_dir / f"{input_state}_to_{target_state}.npy", phi_pred)
        ...
        save_contour_csv(contour, str(contour_dir / f"{input_state}_to_{target_state}_contour20.csv"))
        manifest_rows.append(
            {
                ...
                "duration_s": duration,
                "average_rate": rate,
                "prediction_npy": str(prediction_dir / f"{target_state}.npy"),
            }
        )

    write_prediction_workbook(predictions, str(prediction_dir / "insample_predictions.xlsx"))
    with (prediction_dir / "manifest.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=["process", "cycle", "input_state", "target_state", "duration_s", "average_rate", "prediction_npy"],
        )
        writer.writeheader()
        writer.writerows(manifest_rows)

    if not args.no_plot:
        figure = Path(args.figure) if args.figure else out_dir / "figures" / "insample_zero_contours_1M_to_2E.png"
        if not figure.is_absolute():
            figure = root / figure
        gt_arrays = {
            state: ensure_signed_distance(states[state], level_cfg)
            for state in INSAMPLE_TARGET_STATES
            if state in states
        }
        save_zero_contour_grid(
            predictions,
            str(figure),
            INSAMPLE_TARGET_STATES,
            gt_arrays=gt_arrays,
            title="In-sample one-step prediction phi=0 contours (GT overlay)",
    ...
    print(f"Saved in-sample predictions: {prediction_dir}")
    return 0
